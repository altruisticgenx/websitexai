...
const FeatureCardWithTooltip: React.FC<{
  ...
  return <Tooltip open={isTouched ? true : undefined}>
      <TooltipTrigger asChild>
        ...
        scale: 0.98
      }} onClick={() => setIsTouched(v => !v)} onMouseEnter={() => setIsTouched(false)} style={{
        transformStyle: "preserve-3d",
        perspective: "1000px"
      }} className={cn("group relative w-full overflow-hidden rounded-lg border bg-gradient-to-br backdrop-blur-sm p-2.5 sm:p-3 transition-all cursor-pointer", "touch-manipulation focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50", "hover:shadow-xl shadow-lg", colorClasses.border, colorClasses.gradient, colorClasses.shadow, "before:absolute before:inset-0 before:rounded-lg before:bg-gradient-to-br before:from-white/5 before:to-transparent before:opacity-0 hover:before:opacity-100 before:transition-opacity before:duration-300")} aria-label={`${item.title} – ${item.desc}. Tap to see example.`}>
      ...
      <TooltipContent side="top" className="max-w-[260px] sm:max-w-[320px] bg-slate-900/95 backdrop-blur-sm border-primary/30 p-3" onPointerDownOutside={e => {
      if (isTouched) {
        e.preventDefault();
        setIsTouched(false);
      }
...
const RecentBuilds: React.FC = React.memo(() => {
  ...
  const fetchProjects = useCallback(async () => {
    try {
      ...
      const {
        data,
        error: fetchError
      } = await supabase.from("projects").select("slug, title, sector, summary, tag, image_url").eq("featured", true).order("display_order", {
        ascending: true
      });
  ...
  return <section id="builds" className="relative py-8 sm:py-10 lg:py-16">
      <ParallaxBackground speed={0.6} gradient="from-primary/8 via-accent/8 to-transparent" meshVariant="primary" meshIntensity="medium">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_50%,hsl(var(--primary)/0.15),transparent_50%)]" />
      </ParallaxBackground>

      <div className="<div className=\"relative z-10 w-full max-w-5xl px-3 sm:px-4 mx-auto min-w-0\">\n">
        <motion.div initial={{
        opacity: 0,
        y: 20
      }} whileInView={{
        ...
        {isLoadingProjects ? <div className="mt-6 sm:mt-8">
            <CardsSkeleton />
        ...
        opacity: 1,
        y: 0
      }} className="mt-6 sm:mt-8 rounded-2xl border border-red-400/20 bg-red-400/10 p-6 text-center" role="alert" aria-live="polite">
        ...
        delay: 0.1
            ...
            return <PilotCard key={project.id} id={project.id} title={project.title} sector={project.sector} whoFor={getSectorAudience(project.sector)} problem={getProjectProblem(project.id)} outcome={getProjectOutcome(project.id)} timeToDemo={getTimeToDemo(project.id)} tag={project.tag} imageUrl={imageUrl} />;
          })}
            </PilotCarousel3D>
          </motion.div>}
      </div>
...
const PilotOffer: React.FC = React.memo(() => {
  ...
  return <section id="pilot" className="relative border-t border-slate-900/80 py-10 lg:py-16" aria-labelledby="pilot-heading">
      <ParallaxBackground speed={0.5} gradient="from-secondary/8 via-accent/6 to-transparent" meshVariant="accent" meshIntensity="subtle">
        ...
        delay: 0.1
          ...
          <TooltipProvider delayDuration={160}>
            <div className="grid gap-2 sm:gap-3 grid-cols-2 lg:grid-cols-4">
              {items.map((item, i) => <FeatureCardWithTooltip key={item.title} item={item} index={i} />)}
            </div>
          </TooltipProvider>
        </motion.div>
...
const TypicalProgression: React.FC = React.memo(() => {
  return <section className="relative border-t border-slate-900/80 py-6 sm:py-8 lg:py-12">
      <ParallaxBackground speed={0.4} gradient="from-accent/10 via-primary/8 to-transparent" meshVariant="mixed" meshIntensity="vibrant">
      ...
      <div className="mx-auto w-full max-w-5xl px-3 sm:px-4">
        ...
        }} className={cn("group relative overflow-hidden rounded-xl border-2 p-2.5 sm:p-3 backdrop-blur-sm transition-all", "hover:shadow-lg hover:shadow-current/20", step.ring === "emerald" && "border-emerald-500/40 bg-gradient-to-br from-emerald-500/15 via-teal-500/10 to-emerald-500/15 hover:border-emerald-400/60", step.ring === "blue" && "border-blue-500/40 bg-gradient-to-br from-blue-500/15 via-indigo-500/10 to-blue-500/15 hover:border-blue-400/60", step.ring === "violet" && "border-violet-500/40 bg-gradient-to-br from-violet-500/15 via-purple-500/10 to-violet-500/15 hover:border-violet-400/60", step.ring === "orange" && "border-orange-500/40 bg-gradient-to-br from-orange-500/15 via-amber-500/10 to-orange-500/15 hover:border-orange-400/60")}>
              <div className={cn("absolute -inset-[2px] rounded-xl opacity-0 blur-md transition-opacity duration-300 group-hover:opacity-30", step.ring === "emerald" && "bg-emerald-500", step.ring === "blue" && "bg-blue-500", step.ring === "violet" && "bg-violet-500", step.ring === "orange" && "bg-orange-500")} />
...
const WhoBenefits: React.FC = React.memo(() => {
  ...
  return <section id="benefits" className="relative border-t border-slate-900/80 py-10 lg:py-16">
      <ParallaxBackground speed={0.55} gradient="from-primary/10 via-secondary/8 to-accent/10" meshVariant="secondary" meshIntensity="medium">
...
const AboutMe: React.FC = React.memo(() => {
  return <section id="about" className="relative border-t border-slate-900/80 py-10 lg:py-16">
      <ParallaxBackground speed={0.6} gradient="from-primary/6 via-secondary/8 to-accent/6" meshVariant="mixed" meshIntensity="subtle">
