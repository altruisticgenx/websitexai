...
const PageSection: React.FC<{
  ...
  className,
  children
}) => <section id={id} className={cn("scroll-mt-24 py-8 sm:py-12 lg:py-16", borderTop && "border-t border-border", className)} role="region" aria-labelledby={id ? `${id}-heading` : undefined}>
...
const Stack: React.FC<{
  ...
  className,
  children
}) => <div className={cn(gap === "sm" && "space-y-4", gap === "md" && "space-y-6", gap === "lg" && "space-y-10", className)}>
    {children}
  </div>;
...
const FeatureCardWithTooltip: React.FC<{
  ...
  return <Tooltip open={isTouched ? true : undefined}>
      <TooltipTrigger asChild>
        ...
        scale: 0.98
      }} onClick={() => setIsTouched(v => !v)} onMouseEnter={() => setIsTouched(false)} onBlur={() => setIsTouched(false)} className={cn("group relative w-full overflow-hidden rounded-xl border bg-gradient-to-br backdrop-blur-sm", "p-3 sm:p-4 md:p-5 transition-all cursor-pointer", "ring-1", "focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/60", colorClasses.ring, colorClasses.border, colorClasses.gradient, colorClasses.glow, "before:absolute before:inset-0 before:rounded-xl before:bg-gradient-to-br before:from-white/10 before:via-white/0 before:to-transparent before:opacity-0 hover:before:opacity-100 before:transition-opacity before:duration-300")} aria-expanded={isTouched} aria-label={`${item.title} – ${item.desc}. Tap to see example.`}>
          ...
          <div className="relative z-10 flex items-start gap-2.5">
            ...
            <div className="flex-1 min-w-0 text-left">
              <h4 className="heading-5 mb-1.5 line-clamp-2 text-xs">{item.title}</h4>
...
const RecentBuilds: React.FC = React.memo(() => {
  ...
  return <PageSection id="builds" borderTop>
      <Stack>
        <motion.div initial={{
        opacity: 0,
        y: 14
        ...
        {isLoadingProjects ? <div className="grid gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-4">
            {[...Array(4)].map((_, i) => <div key={i} className="h-64 animate-pulse rounded-2xl bg-muted/60" />)}
        ...
        opacity: 1,
        y: 0
      }} className="rounded-2xl border border-red-400/20 bg-red-400/10 p-6 text-center" role="alert" aria-live="polite">
        ...
        delay: 0.06
          ...
          delay: index * 0.05
                  ...
                  <PilotCard id={project.id} title={project.title} sector={project.sector} whoFor={getSectorAudience(project.sector)} problem={getProjectProblem(project.id)} outcome={getProjectOutcome(project.id)} timeToDemo={getTimeToDemo(project.id)} tag={project.tag} />
                </motion.div>)}
            </motion.div>}
      </Stack>
    </PageSection>;
...
const PilotOffer: React.FC = React.memo(() => {
  return <PageSection id="pilot" borderTop aria-labelledby="pilot-heading">
      <Stack gap="lg">
        <motion.div initial={{
        opacity: 0,
        y: 14
        ...
        delay: 0.05
          ...
          <TooltipProvider delayDuration={160}>
            ...
            }].map((item, i) => <FeatureCardWithTooltip key={item.title} item={item as FeatureItem} index={i} />)}
            </div>
          </TooltipProvider>
        </motion.div>
...
const TypicalProgression: React.FC = React.memo(() => {
  return <PageSection id="progression" borderTop>
      <Stack>
        <motion.div initial={{
        opacity: 0,
        y: 14
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          ...
          duration: 0.45,
          delay: 0.08 * i
        }} className={cn("group rounded-xl border-2 p-5 backdrop-blur-sm transition-all hover:-translate-y-1", step.ring === "emerald" && "border-emerald-500/50 bg-gradient-to-br from-emerald-500/20 to-teal-500/20", step.ring === "blue" && "border-blue-500/50 bg-gradient-to-br from-blue-500/20 to-indigo-500/20", step.ring === "violet" && "border-violet-500/50 bg-gradient-to-br from-violet-500/20 to-purple-500/20", step.ring === "orange" && "border-orange-500/50 bg-gradient-to-br from-orange-500/20 to-amber-500/20")}>
...
const WhoBenefits: React.FC = React.memo(() => {
  ...
  return <PageSection id="benefits" borderTop>
      <Stack>
        <motion.div initial={{
        opacity: 0,
        y: 14
...
const AboutMe: React.FC = React.memo(() => {
  return <PageSection id="about" borderTop>
      <Stack gap="lg">
        <motion.div initial={{
        opacity: 0,
        y: 14
