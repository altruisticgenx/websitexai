...
// NEW layout primitives (assumed added per plan)
...
const Index: React.FC = () => {
  ...
  return (
    <div ref={swipeRef} className="min-h-screen bg-slate-950 text-slate-50 antialiased">
      ...
      {/* Main content gets nav offset to prevent overlap */}
      <main id="main-content" className="pt-16" role="main" aria-busy={isBootLoading}>
        {isBootLoading ? (
          ...
          >
            ...
            {/* Optional: if EngagementModels is meant to render on homepage */}
            <LazySection minHeight={420}>
              <EngagementModels />
            </LazySection>
          </motion.div>
    ...
    <Tooltip open={isTouched ? true : undefined}>
      <TooltipTrigger asChild>
        <motion.button
          ...
          className={cn(
            "group relative w-full overflow-hidden rounded-lg border bg-gradient-to-br backdrop-blur-sm p-3 sm:p-4 transition-all cursor-pointer",
        ...
        >
          ...
          <div className="relative z-10 flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0 text-left">
              <h4 className="heading-6 mb-1 line-clamp-2">{item.title}</h4>
              <p className="body-xs text-muted-foreground leading-snug line-clamp-3">{item.desc}</p>
            </div>
          </div>

          <div className="absolute bottom-1.5 right-2 caption text-muted-foreground/60 sm:hidden">Tap</div>
        </motion.button>
      </TooltipTrigger>
  ...
  return (
    <Section id="builds" border="top">
      <Stack>
        ...
        >
          <h2 className="heading-2 text-foreground">Recent Builds</h2>
...
const PilotOffer: React.FC = React.memo(() => {
  return (
    <Section id="pilot" border="top" aria-labelledby="pilot-heading">
      <Stack gap="lg">
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="rounded-lg border border-slate-800/70 bg-slate-950/50 p-6">
        ...
        <motion.div
          ...
          className="rounded-lg border border-primary/30 bg-gradient-to-br from-primary/5 to-slate-950/80 p-6 backdrop-blur-sm"
        >
...
const TypicalProgression: React.FC = React.memo(() => {
  return (
    <Section id="progression" border="top">
      <Stack>
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {[
            ...
            <motion.div
              ...
              className={cn(
                "group rounded-lg border-2 p-5 backdrop-blur-sm transition-all",
...
const WhoBenefits: React.FC = React.memo(() => {
  ...
  return (
    <Section id="benefits" border="top">
      <Stack>
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
