  ...
  return <>
      ...
      <nav className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80" aria-label="Main navigation">
        ...
        <div className="flex h-16 items-center justify-between">
          {/* Logo/Brand */}
          <a href="#" onClick={handleNavClick} className="flex flex-col font-itim transition-colors hover:text-primary">
          ...
          <div className="hidden md:flex md:items-center md:gap-1" role="navigation" aria-label="Primary">
            <a href="#" onClick={handleNavClick} aria-label="Navigate to home section" aria-current={activeSection === "" ? "page" : undefined} className={cn("px-3 py-1.5 text-xs font-medium lowercase tracking-wide transition-all duration-200 rounded-md", activeSection === "" ? "text-primary bg-primary/10 scale-105" : "text-foreground hover:text-primary hover:bg-accent/50")}>
              home
            </a>
            ...
            <div className="relative">
              <button onClick={() => setIsWorkOpen(!isWorkOpen)} onMouseEnter={() => setIsWorkOpen(true)} aria-expanded={isWorkOpen} aria-haspopup="true" aria-label="Work menu" className={cn("flex items-center gap-1 px-3 py-1.5 text-xs font-medium lowercase tracking-wide transition-all duration-200 rounded-md", isWorkActive ? "text-primary bg-primary/10 scale-105" : "text-foreground hover:text-primary hover:bg-accent/50")}>
                work
                <ChevronDown className={cn("h-3 w-3 transition-transform duration-200", isWorkOpen && "rotate-180")} aria-hidden="true" />
              </button>

              {/* Dropdown Menu */}
              {isWorkOpen && <div onMouseLeave={() => setIsWorkOpen(false)} role="menu" aria-label="Work submenu" className="absolute right-0 top-full mt-2 w-48 rounded-lg border border-border bg-card shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="p-2">
                    <a href="#pilot" onClick={handleNavClick} role="menuitem" aria-label="Navigate to 4-week pilots section" className={cn("block rounded-md px-2.5 py-1.5 text-xs transition-colors", activeSection === "pilot" ? "bg-primary/10 text-primary font-medium" : "text-card-foreground hover:bg-accent hover:text-accent-foreground")}>
                    ...
                    <a href="#builds" onClick={handleNavClick} role="menuitem" aria-label="Navigate to recent builds section" className={cn("block rounded-md px-2.5 py-1.5 text-xs transition-colors", activeSection === "builds" ? "bg-primary/10 text-primary font-medium" : "text-card-foreground hover:bg-accent hover:text-accent-foreground")}>
                    ...
                    <a href="#how" onClick={handleNavClick} role="menuitem" aria-label="Navigate to how it works section" className={cn("block rounded-md px-2.5 py-1.5 text-xs transition-colors", activeSection === "how" ? "bg-primary/10 text-primary font-medium" : "text-card-foreground hover:bg-accent hover:text-accent-foreground")}>
          ...
          {/* Mobile Menu Button */}
          <button ref={mobileMenuButtonRef} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden inline-flex items-center justify-center rounded-md p-2 text-foreground hover:bg-accent hover:text-accent-foreground transition-colors" aria-label="Toggle mobile menu" aria-expanded={isMobileMenuOpen} aria-controls="mobile-menu">
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && <div ref={mobileMenuRef} id="mobile-menu" className="md:hidden border-t border-border/40 py-4 animate-in slide-in-from-top-2 duration-200" role="navigation" aria-label="Mobile navigation">
            <div className="space-y-1">
              <a href="#" onClick={handleNavClick} aria-label="Navigate to home section" aria-current={activeSection === "" ? "page" : undefined} className={cn("block rounded-md px-3 py-2 text-sm font-medium lowercase transition-colors", activeSection === "" ? "text-primary bg-primary/10" : "text-foreground hover:text-primary hover:bg-accent/50")}>
                home
              </a>
              ...
              <div className="space-y-1 pl-4 pt-2">
                ...
                <a href="#pilot" onClick={handleNavClick} aria-label="Navigate to 4-week pilots section" className={cn("block rounded-md px-3 py-2 text-xs transition-colors", activeSection === "pilot" ? "text-primary bg-primary/10 font-medium" : "text-foreground hover:text-primary hover:bg-accent/50")}>
                  4-week pilots
                </a>
                <a href="#builds" onClick={handleNavClick} aria-label="Navigate to recent builds section" className={cn("block rounded-md px-3 py-2 text-xs transition-colors", activeSection === "builds" ? "text-primary bg-primary/10 font-medium" : "text-foreground hover:text-primary hover:bg-accent/50")}>
                  Recent builds
                </a>
                <a href="#how" onClick={handleNavClick} aria-label="Navigate to how it works section" className={cn("block rounded-md px-3 py-2 text-xs transition-colors", activeSection === "how" ? "text-primary bg-primary/10 font-medium" : "text-foreground hover:text-primary hover:bg-accent/50")}>
                  How it works
                </a>
              </div>
