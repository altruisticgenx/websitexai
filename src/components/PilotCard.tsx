...
export function PilotCard({
  ...
  return <Link to={`/case-study/${id}`} className="block group">
      <motion.article ref={cardRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} whileHover={{
      y: -8,
      scale: 1.03,
      transition: {
        duration: 0.35,
      ...
      rotateX,
      rotateY
    }} className={cn("relative flex h-full flex-col rounded-xl border-2 bg-gradient-to-br backdrop-blur-xl p-3 sm:p-4 transition-all duration-500 cursor-pointer overflow-hidden", "shadow-lg", sectorStyles.gradient, sectorStyles.border, sectorStyles.glow, "before:absolute before:inset-0 before:rounded-xl before:bg-gradient-to-br before:from-white/10 before:via-transparent before:to-white/5 before:opacity-0 hover:before:opacity-100 before:transition-opacity before:duration-500", "after:absolute after:inset-0 after:rounded-xl after:bg-gradient-to-t after:from-black/10 after:via-transparent after:to-transparent after:opacity-0 hover:after:opacity-100 after:transition-opacity after:duration-500", className)}>
        {/* Enhanced 3D depth layers with parallax */}
        <motion.div className="absolute inset-0 rounded-xl bg-gradient-to-br from-transparent via-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" style={{
        transform: "translateZ(15px)",
        x: useTransform(mouseX, [-0.5, 0.5], [-10, 10]),
        y: useTransform(mouseY, [-0.5, 0.5], [-10, 10])
      }} />
        <motion.div className="absolute inset-0 rounded-xl bg-gradient-radial from-primary/10 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-md pointer-events-none" style={{
        ...
        {/* Header */}
        <div className="relative z-10 gap-2 mb-2 flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-1">
              <span className={cn("inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-[9px] font-bold shadow-sm", sectorStyles.accent)}>
                <Users className="h-2.5 w-2.5" />
                {whoFor}
              </span>
            </div>
            <h3 className="text-xs font-bold text-foreground line-clamp-2 leading-tight sm:text-xs">
              {title}
            </h3>
            <p className="text-[9px] text-muted-foreground mt-1 line-clamp-1">{sector}</p>
          </div>
          {tag && <span className="rounded-md border border-border/80 bg-muted/90 px-1.5 py-0.5 text-[8px] text-muted-foreground whitespace-nowrap shadow-sm backdrop-blur-sm">
              {tag}
            </span>}
        </div>
        ...
        <div className="relative z-10 mb-2 rounded-lg border-2 border-primary/30 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent backdrop-blur-sm p-2 shadow-inner">
          <div className="flex items-start gap-1.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-foreground/90 leading-snug line-clamp-2 font-medium text-xs">
                {outcome}
              </p>
            </div>
          </div>
