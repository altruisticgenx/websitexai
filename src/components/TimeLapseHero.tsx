...
export function TimeLapseHero() {
  ...
  const glowAnimation = prefersReducedMotion ? {} : {
    filter: ["drop-shadow(0 0 20px rgba(0, 255, 163, 0.3))", "drop-shadow(0 0 30px rgba(0, 255, 163, 0.5))", "drop-shadow(0 0 20px rgba(0, 255, 163, 0.3))"],
    transition: {
      duration: 2,
      repeat: Infinity
    }
  };
  return <div className="relative w-full max-w-5xl mx-auto px-4 py-8 sm:py-12">
      {/* Play/Pause Button */}
      ...
      {/* Visual Canvas */}
      <div ref={swipeRef} className="relative h-[300px] sm:h-[400px] mb-8 rounded-2xl border border-border bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 overflow-hidden">
        {/* Week 0: Chaos State */}
        {visualState === 'chaos' && <motion.div key="chaos" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        opacity: 0
            ...
            <motion.div animate={chaosAnimation} className="absolute top-12 left-16 opacity-70">
              <FileSpreadsheet className="w-12 h-12 sm:w-16 sm:h-16 text-muted-foreground" style={{
            filter: 'blur(1px)'
          }} />
            </motion.div>
            <motion.div animate={chaosAnimation} className="absolute top-24 right-20 opacity-60" style={{
          rotate: '15deg'
        }}>
              <Mail className="w-10 h-10 sm:w-14 sm:h-14 text-muted-foreground" style={{
            filter: 'blur(1px)'
          }} />
            </motion.div>
            <motion.div animate={chaosAnimation} className="absolute bottom-20 left-24 opacity-75" style={{
          rotate: '-12deg'
        }}>
              <FileText className="w-14 h-14 sm:w-18 sm:h-18 text-muted-foreground" style={{
            filter: 'blur(1px)'
          }} />
            </motion.div>
            <motion.div animate={chaosAnimation} className="absolute bottom-16 right-16 opacity-65" style={{
          rotate: '8deg'
        }}>
              <FileSpreadsheet className="w-10 h-10 sm:w-12 sm:h-12 text-muted-foreground" style={{
            filter: 'blur(1px)'
          }} />
            </motion.div>
            <motion.div animate={chaosAnimation} className="absolute top-32 left-1/2 opacity-50" style={{
          rotate: '-20deg'
        }}>
              <Mail className="w-12 h-12 sm:w-16 sm:h-16 text-muted-foreground" style={{
            filter: 'blur(1px)'
          }} />
            </motion.div>

            {/* Label */}
            <motion.div className="absolute bottom-8 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-slate-800/80 backdrop-blur-sm border border-slate-700" initial={{
          opacity: 0,
          y: 10
        }} animate={{
          opacity: 1,
        ...
        {visualState === 'wireframe' && <motion.div key="wireframe" initial={{
        opacity: 0,
        scale: 0.9
      }} animate={{
        opacity: 1,
        ...
        damping: 20
            ...
            <div className="grid grid-cols-3 gap-4 sm:gap-6 max-w-md">
              <motion.div className="aspect-square rounded-lg border-2 border-primary/30 bg-primary/5 flex items-center justify-center" initial={{
            opacity: 0,
            scale: 0
          }} animate={{
            opacity: 1,
            ...
            delay: 0.3
              ...
              <motion.div className="aspect-square rounded-lg border-2 border-primary/30 bg-primary/5 col-span-2" initial={{
            opacity: 0,
            scale: 0
          }} animate={{
            opacity: 1,
            ...
            delay: 0.4
              ...
              <motion.div className="aspect-square rounded-lg border-2 border-primary/30 bg-primary/5" initial={{
            opacity: 0,
            scale: 0
          }} animate={{
            opacity: 1,
            ...
            <motion.div className="absolute bottom-8 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-primary/20 backdrop-blur-sm border border-primary/40" initial={{
          opacity: 0,
          y: 10
        }} animate={{
          opacity: 1,
        ...
        {visualState === 'polished' && <motion.div key="polished" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        ease: "easeOut"
            ...
            {/* Polished dashboard */}
            <motion.div className="relative max-w-lg w-full" animate={glowAnimation}>
        ...
        {isMobile && <motion.div className="absolute top-4 right-4 text-xs text-muted-foreground bg-slate-800/60 backdrop-blur-sm px-2 py-1 rounded" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
      ...
      {/* Timeline Slider */}
      <TimelineSlider position={timelinePosition} onChange={handleManualChange} />
