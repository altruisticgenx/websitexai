...
export const AnimatedCardsStack = ({
  ...
  return <div className="relative h-[280px] w-full md:h-[320px]">
      {activeCards.map((card, index) => {
      return <motion.div key={card.id} className={cn("absolute inset-0 mx-auto flex w-full max-w-lg flex-col justify-between rounded-2xl border border-slate-800/80 bg-slate-900/80 p-4 shadow-lg backdrop-blur-sm md:p-5", card.className)} style={{
...
export const TestimonialsVariant = () => {
  ...
  const cards = testimonials.map(testimonial => ({
    ...
    content: <div className="flex h-full items-center justify-center">
...
export const AwardsVariant = () => {
  const awards = [{
    ...
    year: "2024",
    description: "Recognized for innovative energy analytics platform that identified $180k+ in savings opportunities across 200+ buildings."
  }, {
    id: 2,
    title: "EdTech Innovation Grant",
    organization: "National Education Foundation",
    year: "2023",
    description: "Awarded for pilot management system that improved outcome tracking and enabled data-driven program decisions."
  }, {
    id: 3,
    title: "Climate Tech Builder",
    organization: "Cleantech Accelerator",
    year: "2024",
    description: "Selected for impact-driven approach to building tools that help organizations reduce carbon footprint through better data."
  }];
  const cards = awards.map(award => ({
    id: award.id,
    content: <div className="flex h-full flex-col justify-between">
        <div>
...
export const MetricsVariant = () => {
  const metrics = [{
    ...
    value: "100%",
    description: "Every pilot client has continued to the next phase or recommended us to their network.",
    context: "Based on 12+ projects across energy, education, and civic sectors"
  }, {
    id: 2,
    title: "Time to First Value",
    value: "1 week",
    description: "First meaningful deliverable shipped in Week 1. No 3-month discovery phases.",
    context: "Average across all pilot projects since 2023"
  }, {
    id: 3,
    title: "Cost Savings Identified",
    value: "$500k+",
    description: "Total operational savings surfaced through energy analytics and process optimization tools.",
...
export const CaseStudiesStack = ({
  ...
  const getProjectIcon = (slug: string) => {
    switch (slug) {
      ...
      case "founder-os-dashboard":
        return <svg className="h-3 w-3 sm:h-4 sm:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return <div className="relative h-[260px] w-full sm:h-[300px] md:h-[360px] touch-pan-y" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      {activeCards.map((study, index) => {
      const isHovered = hoveredIndex === index;
      const isTop = index === 0;
      return <motion.div key={study.id} className="absolute inset-0 mx-auto flex w-full max-w-lg" style={{
        transformOrigin: "top center",
        perspective: "1000px",
        ...
        damping: 30
            ...
            <Link to={`/case-study/${study.id}`} className="flex h-full cursor-pointer flex-col justify-between rounded-2xl border border-blue-400/20 bg-gradient-to-br from-slate-900/95 to-slate-950/95 p-3.5 shadow-2xl backdrop-blur-md transition-all hover:border-blue-400/40 sm:rounded-3xl sm:p-5 md:p-6">
              <div className="space-y-2.5 sm:space-y-3">
              <motion.div className="inline-flex items-center gap-1.5 rounded-full border border-blue-400/30 bg-blue-400/10 px-2.5 py-1 text-[10px] font-medium text-blue-300 sm:gap-2 sm:px-3 sm:py-1.5 sm:text-xs" animate={isTop && isHovered ? {
              scale: 1.05
            } : {
              scale: 1
            }} transition={{
            ...
            <div className="mt-2.5 flex items-center justify-between sm:mt-3">
              <motion.span className="rounded-full border border-slate-700/80 bg-slate-800/50 px-2.5 py-1 text-[10px] font-medium text-slate-300 sm:px-3 sm:py-1.5 sm:text-xs" animate={isTop && isHovered ? {
              x: 3
            } : {
              x: 0
            }} transition={{
