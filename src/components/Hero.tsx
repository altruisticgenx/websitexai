...
export function Hero() {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLElement>) => {
    ...
    const newParticle: TrailParticle = { id: particleIdRef.current++, x, y, timestamp: now, speed };
  ...
  return (
    <section 
      ref={ref} 
      id="home" 
      className="relative py-8 sm:py-12 lg:py-16 overflow-hidden bg-slate-950" 
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <motion.div 
          ...
          animate={{
            background: [
              'radial-gradient(circle at 20% 50%, hsl(var(--primary) / 0.25) 0%, transparent 50%), radial-gradient(circle at 80% 50%, hsl(var(--accent) / 0.2) 0%, transparent 50%)',
              'radial-gradient(circle at 80% 30%, hsl(var(--primary) / 0.3) 0%, transparent 50%), radial-gradient(circle at 20% 70%, hsl(var(--accent) / 0.25) 0%, transparent 50%)',
              'radial-gradient(circle at 50% 80%, hsl(var(--primary) / 0.25) 0%, transparent 50%), radial-gradient(circle at 50% 20%, hsl(var(--accent) / 0.2) 0%, transparent 50%)',
              'radial-gradient(circle at 20% 50%, hsl(var(--primary) / 0.25) 0%, transparent 50%), radial-gradient(circle at 80% 50%, hsl(var(--accent) / 0.2) 0%, transparent 50%)',
            ]
          }}
              ...
              <motion.div 
                ...
                style={{
                  ...
                  background: speedRatio > 0.5 
                    ? `radial-gradient(circle, hsl(var(--accent) / ${0.7 * (1 - progress)}), transparent)`
                    : `radial-gradient(circle, hsl(var(--primary) / ${0.6 * (1 - progress)}), transparent)`,
      ...
      <motion.div 
        className="relative z-10 px-4 py-6 sm:py-8 mx-auto max-w-4xl" 
        style={{ y: yForeground, opacity }}
      >
        {/* 3D glass card with enhanced depth */}
        <motion.div 
          className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.07] via-white/[0.03] to-transparent backdrop-blur-2xl shadow-2xl"
          whileHover={{
            scale: 1.01,
        ...
        >
          ...
          <div className="relative p-6 sm:p-8 lg:p-10 space-y-4 sm:space-y-5">
            ...
            <motion.div 
              className="inline-flex items-center gap-2 rounded-full border-2 border-primary/60 bg-gradient-to-r from-primary/15 via-primary/10 to-accent/15 backdrop-blur-xl px-3 py-1.5 text-[10px] sm:text-xs font-bold text-foreground shadow-[0_0_25px_hsl(var(--primary)/0.3)]" 
            ...
            >
              <motion.span 
                className="inline-flex h-2 w-2 rounded-full bg-primary shadow-[0_0_15px_hsl(var(--primary))]" 
                animate={{
                  opacity: [1, 0.3, 1],
                  scale: [1, 1.5, 1],
              ...
              />
              <span className="font-mono tracking-wide">{'>'} Ship pilot-ready AI tech</span>
            </motion.div>

            {/* Main headline - vibrant 3D text */}
            <motion.h1 
              className="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight leading-tight bg-gradient-to-br from-foreground via-foreground to-muted-foreground bg-clip-text text-transparent" 
            ...
            {/* Subhead */}
            <motion.p 
              className="text-xs sm:text-sm lg:text-base max-w-2xl text-muted-foreground leading-relaxed" 
            ...
            <motion.div 
              className="flex flex-wrap gap-2 sm:gap-3 items-center text-xs sm:text-sm" 
              initial={{ opacity: 0, y: 15 }} 
              animate={{ opacity: 1, y: 0 }} 
            ...
            >
              ...
              {["energy", "education"].map((sector) => (
                <motion.button 
                  key={sector}
                  className="relative px-3 py-2 rounded-xl border-2 border-primary/60 bg-gradient-to-br from-primary/15 via-primary/10 to-transparent backdrop-blur-xl text-foreground font-bold overflow-visible group transition-all shadow-[0_0_20px_hsl(var(--primary)/0.2)]"
                  onMouseEnter={() => setActiveSector(sector as "energy" | "education")} 
                ...
                >
                  {/* Glowing halo */}
                  <motion.div 
                    className="absolute inset-0 rounded-xl bg-primary blur-2xl -z-10" 
                    animate={{
                  ...
                  {activeSector === sector && (
                    <>
                      {[...Array(6)].map((_, i) => (
                        <motion.div
                          key={i}
                          className="absolute w-1 h-1 rounded-full bg-primary shadow-[0_0_8px_hsl(var(--primary))]"
                          style={{
                            left: "50%",
                            top: "50%",
                          }}
                  ...
                  <span className="relative z-10 capitalize">{sector}</span>
                </motion.button>
              ))}
            </motion.div>
            ...
            >
              <motion.a
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border-2 border-primary bg-gradient-to-br from-primary via-primary/90 to-primary/80 px-5 py-3 text-sm font-bold text-primary-foreground shadow-[0_0_30px_hsl(var(--primary)/0.4)] backdrop-blur-xl transition-all"
                whileHover={{
                  scale: 1.05,
              ...
              <motion.a
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border-2 border-border bg-gradient-to-br from-muted/80 via-muted/60 to-muted/40 px-5 py-3 text-sm font-bold text-foreground shadow-lg backdrop-blur-xl transition-all hover:border-primary/50"
                whileHover={{
                  scale: 1.05,
            ...
            {/* Trust line */}
            <motion.p 
              className="text-[10px] sm:text-xs text-muted-foreground/80 pt-2" 
              initial={{ opacity: 0 }} 
              animate={{ opacity: 1 }} 
