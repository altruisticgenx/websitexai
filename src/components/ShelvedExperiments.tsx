    ...
    <Section id="shelved" spacing="compact" border="top">
      <Stack gap="md">
        <header className="max-w-2xl">
          <h2 className="heading-4">The Lab Morgue</h2>
          ...
          {experiments.map((exp, index) => {
            ...
            return (
              <motion.div
                ...
                onClick={() => setExpandedCard(isExpanded ? null : exp.name)}
                ...
                >
                  ...
                  <div className="absolute top-4 right-4 -rotate-12 opacity-30">
                    <div className={`px-3 py-1 border-2 ${
                      exp.statusColor === 'red' ? 'border-red-600' : 'border-amber-600'
                    } rounded text-xs font-bold tracking-wider`}>
                      ARCHIVED
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 mb-4">
                    <div className={`p-2 rounded-lg ${
                      exp.statusColor === 'red' ? 'bg-red-900/20' : 'bg-amber-900/20'
                    }`}>
                      <Icon className={`w-5 h-5 ${
                        exp.statusColor === 'red' ? 'text-red-400' : 'text-amber-400'
                      }`} />
                    </div>
                  ...
                  <motion.div
                    initial={{ rotate: -5, y: -20, opacity: 0, scale: 0.9 }}
                    whileHover={isHovered ? { 
                      rotate: -3, 
                      y: 0, 
                      opacity: 1,
                    ...
                    className="absolute top-6 right-6 bg-yellow-100 p-3 shadow-xl rounded-sm max-w-[140px] pointer-events-none"
                    style={{ 
                  ...
                  {/* Expand indicator */}
                  {!isExpanded && (
                    <div className="absolute bottom-3 right-3 text-[10px] text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                      Click to expand
                    </div>
                  )}
                </motion.div>
